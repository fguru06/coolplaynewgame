...
const socket = io("https://your-project-name.vercel.app"); // Replace with your Vercel URL
...
socket.on("updatePlayers", (players) => {
	...
	for (const [id, player] of Object.entries(players)) {
		...
		if (id !== socket.id) {
			const otherPlayer = new component(70, 100, null, player.x, player.y);
			otherPlayer.update();
		}
	}
});
			...
			if (score < 15) {
				...
				setTimeout(function () {
					...
					gameOverImage.src = isPeach
						? "public/images/Game_Over_Peach.png"
						: isLuigi
						? "public/images/Game_Over_Luigi.png"
						: "public/images/Game_Over.png";
...
// Update the server with the player's score
function updateScore() {
	socket.emit("updateScore", score); // Send the current score to the server
...
function Congrats() {
	...
	myGameArea.canvas.style.backgroundImage = "url('public/images/Congrats.png')";
	if (!document.getElementById("restart-button")) {
